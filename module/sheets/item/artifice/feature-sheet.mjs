...
export class UtopiaArtificeFeatureSheet extends api.HandlebarsApplicationMixin(sheets.ItemSheetV2) {
  constructor(options = {}) {
    super(options);
  }

  static DEFAULT_OPTIONS = {
    classes: ["utopia", "artifice-feature-sheet"],
    position: {
      width: 600,
      height: "auto",
    },
  ...
  static PARTS = {
    ...
    attributes: {
      template: "systems/utopia/templates/item/artifice/feature/attributes.hbs",
    },
    description: {
  ...
  async _onRender() {
    const optionsInput = this.element.querySelectorAll(".crafting-prompt-input");
    if (optionsInput) {
      optionsInput.forEach(async (option) => {
        this._fixCraftingPrompt(option, context);
      });
    }
  }  

  async _fixCraftingPrompt(option, context) {
    const prompt = this.document.system.craftingPrompt;
    option.value = prompt;
