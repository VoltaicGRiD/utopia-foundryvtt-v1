...
export class UtopiaArmorSheet extends api.HandlebarsApplicationMixin(
  sheets.ItemSheetV2
) {
  constructor(options = {}) {
    super(options);
  ...
  static DEFAULT_OPTIONS = {
    ...
    window: {
      title: "UTOPIA.SheetLabels.armor",
    },
  ...
  static PARTS = {
    ...
    attributes: {
      template: "systems/utopia/templates/item/gear/armor.hbs",
  ...
  async _prepareContext(options) {
    ...
    context.features = [];
    for (const feature of this.item.system.features) {
    ...
    await Promise.all(itemPacks.map(async p => {
      const docs = await p.getDocuments();
      const gearFeatures = docs.filter(d => d.type === "gearFeature");
      await Promise.all(gearFeatures.map(async doc => {
        const item = await fromUuid(doc.uuid);
        context.availableFeatures.push(item);
      }));
    }));
  ...
  static async _addFeature(event) {
    const feature = this.element.querySelector('#searchFeatures').value;
  ...
  static async _deleteFeature(event) {
