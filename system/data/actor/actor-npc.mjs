...
export default class UtopiaNPC extends UtopiaActorBase {
  ...
  static defineSchema() {
    ...
    schema.automated = new fields.BooleanField({
      required: true,
      nullable: false,
      initial: true
    });
    schema.overrides = new fields.ObjectField();
    ...
    *allApplicableEffects() {
    ...
    async _handleOverrides(overrideFlags) {
      let entries = Object.entries(overrideFlags);
      for (let [key, value] of entries) {
        console.log(key, value);
        let path = key;
        if (key.includes("system.")) {
          path = key.split("system.")[1];
        }
      ...
      entries = Object.entries(this.overrides);
      for (let [key, value] of entries) {
        let path = key;
        if (key.includes("system.")) {
          path = key.split("system.")[1];
